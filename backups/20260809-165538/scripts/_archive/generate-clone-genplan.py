#!/usr/bin/env python3
"""Génère le clone gen-plan v3.6.1 + écosystème et calcule son SHA-256."""
import hashlib

CLONE = r"""# CLONE DE DISCUSSION — gen-plan v3.6.1 et Écosystème Knowledge
...
# Calcul SHA-256
sha256 = hashlib.sha256(CLONE.encode('utf-8')).hexdigest()
final = CLONE.replace('{sha256}', sha256)

output_path = '/home/z/my-project/download/clone-de-gen-plan-3.6-et-ecosystem.md'
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(final)

print(f'Clone généré : {output_path}')
print(f'SHA-256 : {sha256}')
print(f'Taille : {len(final)} caractères, {final.count(chr(10))+1} lignes')

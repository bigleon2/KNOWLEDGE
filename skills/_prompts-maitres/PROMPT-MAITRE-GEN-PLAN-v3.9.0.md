# PROMPT MAÎTRE — Installation du skill gen-plan v3.9.0

> **Version du prompt** : 1.4.0
> **Skill cible** : gen-plan v3.9.0
> **Date** : 2026-09-06
...
**Matérialisation des disciplines (2026-09-06, correction A11)** : les 4 disciplines d'exécution (context, loop, graph, harness engineering) sont matérialisées en agents autonomes (`{{SKILLS_ROOT}}_disciplines/*.agent`, format §2.3 autonomous-agent) et la discipline prompt-engineering en skill spécialisé (`agent-prompt-engineering`). La matérialisation antérieure `references/gen-plan.agent` (session A9) est **retirée** : elle matérialisait gen-plan lui-même au lieu des disciplines (erreur d'interprétation corrigée à la demande de l'utilisateur). gen-plan demeure un skill ; sa couche agent est assurée par les 4 agents de discipline qu'il mobilise et par sa couche evals.

---

### §1.10 Pipeline d'optimisation écosystème (Z0-Z6)

Transféré de la directive utilisateur « Pipeline optimisation écosystème » (2026-09-06) ; formalisé en v3.8.0. Ce pipeline définit les opérations de maintenance et d'optimisation du corpus : gen-plan le mobilise pour toute tâche d'optimisation de l'écosystème (l'opérationnalisation en tant qu'agent des disciplines est assurée par `{{SKILLS_ROOT}}_disciplines/`, cf. §1.9). Les vérifications Z4 mobilisent les arbitres existants (verify-cross, verify-correct-work, spell-check, sync-download) et les hooks correct-work.

...
- **Répertoires** : kebab-case (`gen-plan`, `correct-work`, `clone-chat`). Exception : le préfixe `_` est réservé aux dossiers d'infrastructure (`_prompts-maitres/` = pas un skill, pas de `SKILL.md` ; `_prompts-maitres/_archive/` = archive des anciens PMs, pas des skills actifs). Note session A12 : les matérialisations agent `_disciplines/` (session A11) sont retirées — les 4 disciplines d'exécution sont désormais des skills actifs à part entière (voir §7).
